...
export function AnnualPayrollClient({
  ...
  const statusConfig: Record<string, { bg: string; text: string; border: string; label: string }> = {
    draft:             { bg: 'bg-slate-50',    text: 'text-slate-500',  border: 'border-slate-200', label: t('annualStatusDraft') },
    hr_reviewed:       { bg: 'bg-yellow-50',   text: 'text-yellow-800', border: 'border-yellow-200', label: t('annualStatusHrReviewed') },
    finance_confirmed: { bg: 'bg-blue-50',     text: 'text-blue-800',   border: 'border-blue-200',   label: t('annualStatusFinanceConfirmed') },
    coo_approved:      { bg: 'bg-green-50',    text: 'text-green-800',  border: 'border-green-200',  label: t('annualStatusCooApproved') },
    paid:              { bg: 'bg-emerald-50',  text: 'text-emerald-800', border: 'border-emerald-200', label: t('annualStatusPaid') },
    rejected:          { bg: 'bg-red-50',      text: 'text-red-800',    border: 'border-red-200',    label: t('annualStatusRejected') },
  }
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={String(selectedYear)} onValueChange={(v) => handleYearChange(v ?? '')}>
            ...
            <SelectContent>
              {yearOptions.map(y => (
                <SelectItem key={y} value={String(y)}>{t('annualYearSuffix', { year: y })}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        {isHR && (
          <div className="flex items-center gap-2">
            <label htmlFor="user-select" className="text-sm font-medium text-slate-600 dark:text-slate-400">
              {t('annualEmployeeLabel')}
            </label>
            <Select value={selectedUserId} onValueChange={(v) => setSelectedUserId(v ?? '')}>
              <SelectTrigger id="user-select" className="w-44">
                <SelectValue placeholder={t('annualSelectEmployee')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={currentUser?.id ?? ''}>
                  {currentUser?.display_name}{t('annualMe')}
                </SelectItem>
                {allUsers
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl border border-emerald-100 bg-emerald-50 dark:bg-emerald-950/30 dark:border-emerald-900 p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-emerald-700 dark:text-emerald-400">{t('annualGrossIncome')}</span>
          </div>
          ...
          <p className="text-xs text-emerald-600/70 dark:text-emerald-500 mt-1">{t('annualBasePlusOtBonus')}</p>
        </div>
        ...
        <div className="rounded-xl border border-red-100 bg-red-50 dark:bg-red-950/30 dark:border-red-900 p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-red-700 dark:text-red-400">{t('annualDeductions')}</span>
          </div>
          ...
          <p className="text-xs text-red-600/70 dark:text-red-500 mt-1">{t('annualInsuranceTaxEtc')}</p>
        </div>
        ...
        <div className="rounded-xl border border-emerald-200 bg-white dark:bg-slate-800 dark:border-emerald-800 p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-emerald-700 dark:text-emerald-400">{t('annualNetPay')}</span>
          </div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{t('annualActualTotal')}</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden shadow-sm">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <h2 className="text-base font-semibold text-slate-800 dark:text-slate-200 font-[Lexend]">
            {t('annualMonthlyBreakdown', { year: selectedYear })}
          </h2>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-700/50">
                <th className="text-left px-4 py-3 font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{tc('month')}</th>
                <th className="text-right px-4 py-3 font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{t('baseSalary')}</th>
                <th className="text-right px-4 py-3 font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{t('overtimePay')}</th>
                <th className="text-right px-4 py-3 font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{t('bonus')}</th>
                <th className="text-right px-4 py-3 font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{t('deductions')}</th>
                <th className="text-right px-4 py-3 font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{t('netPay')}</th>
                <th className="text-left px-4 py-3 font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{tc('status')}</th>
              </tr>
            </thead>
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">
                      {t('annualMonthSuffix', { month })}
                      {isCurrentMonth && (
                        <span className="ml-2 text-xs text-emerald-600 dark:text-emerald-400 font-normal">{t('annualCurrentMonth')}</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      {r ? (
                        <StatusBadge status={r.status} statusConfig={statusConfig} />
                      ) : (
            ...
            <tfoot>
              <tr className="bg-emerald-50 dark:bg-emerald-950/30 border-t-2 border-emerald-200 dark:border-emerald-800">
                <td className="px-4 py-3 font-bold text-slate-800 dark:text-slate-200 whitespace-nowrap">{t('annualTotal')}</td>
        ...
        {!totals.hasAny && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
              {t('annualNoRecords', { year: selectedYear })}
            </p>
...
function StatusBadge({ status, statusConfig }: { status: string; statusConfig: Record<string, { bg: string; text: string; border: string; label: string }> }) {

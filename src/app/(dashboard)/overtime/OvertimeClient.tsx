    ...
    <div className="space-y-4">
      ...
      <Dialog open={applyOpen} onOpenChange={setApplyOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{t('newOvertimeApplication')}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('overtimeStartTime')}</label>
                <Input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="mt-1" />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('overtimeEndTime')}</label>
                <Input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} className="mt-1" />
              </div>
            </div>
            {hours && <p className="text-sm text-slate-500">{t('totalHours', { hours })}</p>}
            {projects.length > 0 && (
              <div>
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('relatedProject')}</label>
            ...
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('overtimeReason')}</label>
  ...
  return (
    <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{t('rejectOvertimeTitle')}</DialogTitle></DialogHeader>

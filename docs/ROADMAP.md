...
### 已完成模組的加強子項
- [ ] **資產**：採購驗收單（GR）一鍵轉資產（schema 已留 `source_gr_id`，只差 UI）— 🔨 **先做**（2026-07-06 Luna）
...
### 舊 PRD 承諾但從未實作（2026-07-05 稽核掃描發現）— ✅ 全部核准做（2026-07-06 Luna）
- [ ] **合約到期自動提醒**（90/30 天）— 設定值 `contract_reminder_days_first/second` 已存在但無任何 cron 消費；牽涉法遵/稽核，**建議優先**
- [ ] **合約核准後通知營運長** + **Teams bot 合約審核結果通知**（documents 審核路由目前核准/退回後無任何通知）
- [ ] **上傳合約時「同公司有 X 份文件，是否關聯？」提示** + `documents.related_doc_id` 顯式綁定（詳情頁的同公司關聯顯示已做，缺上傳互動）
- [ ] **公告發布者報表「一鍵催人」**（報表本體已做，nudge 動作未做）
- [ ] **公告簽署清單 xlsx 匯出**（`/api/export` 目前只有 attendance/expenses/leave/payroll）
- [ ] **公告未確認提醒的自訂頻率**（`documents.reminder_days` 發布時寫入但 daily-digest 未讀取）
- [ ] **專案加班「營運長超額通知」**（填報與審核已完成，超額通知未做）

...
- [ ] **行事曆**：Outlook **單向**同步（myOPS→Outlook），**每人自己的 Microsoft OAuth**，**核准後自動推送**（2026-07-07 Luna 定案；澄清為單向：核准建事件、Outlook 端刪除不回寫）。作法：登入 callback 擷取並存 refresh token → 請假/出差核准時以當事人身分在其 Outlook 建/刪事件

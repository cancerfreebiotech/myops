...
### 已完成模組的加強子項
- [ ] **資產**：採購驗收單（GR）一鍵轉資產（schema 已留 `source_gr_id`，只差 UI）
- [ ] **行事曆**：Outlook/Teams 日曆雙向同步
- [ ] **營運儀表板**：薪資成本圖
- [ ] **AI 政策問答升級**：pgvector + embedding 檢索（取代全文塞 prompt 的 60000 字截斷）；PDF 文字抽取（目前純 PDF 文件讀不到）

### 舊 PRD 承諾但從未實作（2026-07-05 稽核掃描發現）

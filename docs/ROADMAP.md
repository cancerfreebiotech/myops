...
### v0.8.4 逐模組深掃（2026-07-11）後的待決 low/medium（已修 critical+high+多數 medium）
> 完整發現與觸發路徑存於審查輸出；以下為刻意延後的低風險項，可逐一處理。
- [ ] [medium] leave：請假行事曆初次載入只看得到自己的假，與換月後（RPC 抓全部門）不一致
- [ ] [low] overtime：核准/退回缺少 status 條件的 UPDATE，存在雙重核准 TOCTOU
- [ ] [low] overtime：COO 身分判定不一致：通知用 job_role='coo'，核准權限用 has_feature('coo_notify')
- [ ] [low] payroll：異常偵測頁與其 API 權限判定不一致，coo/finance 進頁卻掃描 403
- [ ] [low] payroll：無薪假扣款以 start_date 落在當月且整段 total_days 計入，跨月請假會算錯
- [ ] [low] contracts：審核 PATCH 無狀態轉移把關，可任意回退/跳關狀態並重複觸發 COO 通知
- [ ] [low] procurement：同一來源單可轉出多張請款單，無防重（重複請款/付款風險）
- [ ] [low] procurement：AP→分期請款單期數指派存在並發競態，可產生重複 installment_no
- [ ] [low] assets：PATCH 缺少 POST 已有的欄位驗證，非法值直接落到 DB 並外洩 Postgres 錯誤訊息
- [ ] [low] assets：checkout/checkin 記錄未保存借用人，借用/歸還歷史無法從 audit trail 還原
- [ ] [low] assets：assets API 未檢查 feature.assets flag，模組被關閉時仍可透過 API 讀寫
- [ ] [low] lab：入庫(POST lots)非原子：批次與 audit log 分兩次 insert，失敗會產生孤兒批次且重試造成重複批號
- [ ] [low] daily-report：daily-report API 路由完全未檢查 feature flag，關閉功能後仍可用
- [ ] [low] daily-report：KPI 輸入在 600ms debounce 內切換日期會靜默丟失未存的數值
- [ ] [low] calendar：PATCH company_events 缺伺服端欄位驗證（可經直打 API 寫入空白 title）
- [ ] [medium] approvals：補打卡簽核中心可見範圍用 approver_id 快照，與實際核准權限（即時 manager_id）不一致
- [ ] [low] lifecycle-recruiting：已完成 checklist 的項目仍可取消勾選，狀態不會回退，造成「已完成卻未 100%」的不一致
- [ ] [low] lifecycle-recruiting：lifecycle / recruiting API 只檢查角色不檢查 feature flag，關閉功能旗標無法真正停用 hr_manager 的 API 存取
- [ ] [low] admin-settings-audit-feedback：審計動作篩選選項與 DB 實際 action 值不符，翻譯/OCR 等日誌無法篩選且顯示原始字串

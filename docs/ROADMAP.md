...
- [ ] **行事曆**：Outlook 日曆**雙向**同步、**每人自己的 Microsoft OAuth**（2026-07-07 Luna 定案）。基礎已具備：OAuth 已要 `Calendars.ReadWrite offline_access`、`/api/calendar/leave-event` 已能以 session token 建 Graph 事件、leave_requests.outlook_event_id 已存。缺口見下方「行事曆雙向同步實作計畫」
...
### 明確標記「未來再評估」
- [ ] 文件 OCR 全文搜尋；彈性工時班別管理；打卡 GPS 地理圍欄（皆舊 PRD 排除清單項）

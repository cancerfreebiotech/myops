...
| 2 | **統一簽核中心** | `/approvals` 彙總待我審批：請假、加班、報帳、文件/合約、薪資、採購；一鍵核准；Dashboard 待辦卡 | 🔨 **進行中** |
| 3 | **資產與儀器管理** | 資產台帳、領用/歸還、保養與校驗排程＋到期提醒（ISO/TFDA 稽核需求）；採購驗收單一鍵轉資產 | ⬜ 排隊中 |
| 4 | **教育訓練與證照** | 訓練時數記錄、必修指派＋閱讀確認、證照（GCP/生安/輻安）到期提醒、HR 總覽 | ⬜ 排隊中 |
...
- **補打卡申請沒有審批 endpoint**：`attendance_makeup_requests` 建立後無任何 API 可核准 — 需補 PATCH（規劃於簽核中心或獨立修復）
- **薪資權限 key 不一致**：程式檢查 `view_payroll`/`confirm_payroll`/`approve_payroll`，但可指派清單（`src/lib/features.ts`）只有 `payroll_view` — admin UI 指派的權限無效，需對齊

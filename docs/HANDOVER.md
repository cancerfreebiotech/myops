...
> **2026-07-07 更新**：roadmap 可做項目已全部完成 —— #11 績效考核（v0.6.1）、GR 轉資產（v0.6.5）、舊 PRD 未實作的 B 類 7 項（v0.6.5：合約到期提醒/核准通知/文件關聯、公告催人/匯出/頻率、專案加班超額通知）皆已上線。**唯一剩下：行事曆 Outlook/Teams 同步**（已核准做，但需先拍板單向/雙向與授權模型 —— 見 ROADMAP）。#13 服務台 Luna 決定不做。

### 尚未做的功能

| # | 功能 | 狀態 / 觸發條件 |
|---|------|----------------|
| — | **行事曆 Outlook/Teams 同步** | 唯一待做。需決策：單向推送 vs 雙向、per-user OAuth vs 服務帳號 |

...
1. **是否先停止開發、進入 UAT 實測**（強烈建議）。
2. #11 績效、#13 服務台要不要現在做（兩者 roadmap 觸發條件都未達成）。
3. PM 舊站（每日報告前身）歷史資料是否遷移 — 目前每日報告從零開始用，舊資料未遷移，也沒拿到舊站 Supabase 憑證。

## 未寄的 release note

**v0.5.9 和 v0.6.0 的 release note 尚未寄**（v0.5.5–v0.5.8 已寄）。Luna 說要寄時，用 `/notify-release` 或手寫 script，涵蓋這兩版（入職離職流程、招募、試劑耗材）。

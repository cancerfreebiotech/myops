...
11. **⚠️ 授權述詞必須 fail-closed（2026-07-29 修復的系統性 bug）**：`SELECT expr FROM users WHERE id = auth.uid()` 在查不到列時回 **NULL 不是 false**。判定寫在 SQL `WHERE` 裡是安全的（NULL 不為真 → 濾掉），但寫成 PL/pgSQL `IF NOT (is_admin() OR has_feature(...)) THEN RAISE` 就 **fail open**——`NOT NULL` = NULL，而 `IF NULL THEN` 不進入分支，RAISE 被跳過、閘門形同不存在。`is_admin()` / `has_feature()` 已包 `COALESCE(..., false)`（migration `20260729000001`），新寫的述詞也必須這樣包，且 `granted_features` 要 `COALESCE(..., '{}')`（`x = ANY(NULL)` 同樣是 NULL）。
12. **⚠️ 函式 GRANT/REVOKE 陷阱**：Supabase 對 public schema 的新函式有預設授權，會建立一筆 **PUBLIC** 授權（`proacl` 裡開頭是 `=X/postgres` 那筆）**以及**具名的 anon/authenticated 授權。兩者獨立，**只 REVOKE 其中一邊收不掉**——只寫 `FROM anon` 時 anon 仍會透過 PUBLIC 繼承（2026-07-29 實際踩到：REVOKE anon 後以公開 anon key 實測仍回 200）。一律寫 `REVOKE ALL ON FUNCTION f(args) FROM PUBLIC, anon, authenticated;` 再明確 `GRANT` 回需要的角色。**`DROP` 後重建函式會重新套用預設授權**，重建時務必再收一次。
    判斷準則：函式內部**沒有** `auth.uid()`/`is_admin()`/`has_feature()` 檢查 ⇒ 只能 `GRANT service_role`。RLS policy 用到的 helper 述詞（`is_manager_of`、`dr_is_*`、`dr_can_manage_task`、`dr_shares_group_with_me`）**必須保留 `authenticated`**，否則 policy 評估失敗、整個 RLS 壞掉。
    **驗證方式**：改完 ACL 後，用 `.env.local` 的 anon key 直打 PostgREST `POST /rest/v1/rpc/<fn>` 實測應回 401；只查 `pg_proc.proacl` 不夠（PUBLIC 那筆很容易看漏）。

## 運維流程

- **跑 migration**（pooler 直連不通，走 Management API）：讀 `.env.local` 的 `SUPABASE_ACCESS_TOKEN`，`POST https://api.supabase.com/v1/projects/odzwvkhdrahomgqwlwba/database/query`，body `{ "query": "<SQL>" }`，回 `201 []` 即成功。**線上 migration 由 Po 授權即可**（2026-07-22 起改為此規則，不再需要 Luna；auto mode 的分類器仍可能擋，重試即可）；安全作法：先 push 程式碼（flag 關著沒風險），migration 等授權再跑。改動 DB 行為後，慣例是包在 `BEGIN … ROLLBACK` 裡先實測（含負面測試：越權情境必須被擋），再正式套用。
...
- **Release 通知**：`/notify-release` skill（設定在 `.claude/notify-release.config.json`，憑證 `~/.claude/notify-release.env`，走 FlightPath relay `POST /api/systems/send-email`）。**不要自動寄、不要每次 push 就寄**——累積起來等 Po 說「notify release」才寄，一封涵蓋上次寄出之後的所有版本，且**信中必須列出對應的 feedback ticket id 與處理結果**（Po 明確要求）。收件人為全體 active users（單一 personalization，收件人互見，內部通知刻意如此）。CHANGELOG 的 `## v{VERSION}` 必須對得上 `package.json` 版號，否則 skill 會 abort。

...
- **「部分工時」那幾級目前會被系統丟掉——這是我們的 bug，會修**：您上傳的檔案在等級 1 之前有 17 列「部分工時」（投保薪資 11,100～28,590），但等級欄位不是數字，系統的匯入程式把它們當成無效資料**靜默略過**，只收了編號 1 以後的 58 級（從 29,500 起）。系統也沒有告訴您略過了幾列——這是我們該修的兩件事：**接受部分工時級距**，以及**任何被略過的列都要明白顯示**。
  **目前沒有影響任何人的薪資**：現職 8 位同事都是全職，批次計算也只納入全職，所以不會有人被套到錯的級距。但若之後有部分工時同事，投保級距就會抓到最低的 29,500，保費會算多。
- **不需要另外上傳「薪資對級距」的對照表**：系統就是拿實際薪資去比您上傳的「投保薪資」欄位，取**第一個大於或等於該薪資的級距**（超過最高級距就用最高級）。所以只要級距表本身完整（含部分工時那幾級），對照就會自動成立。
- **勞退目前確實沒有地方上傳，會補**：現在的勞退提繳是直接用「月薪 × 6%」計算，沒有走「勞工退休金月提繳工資分級表」。這是尚未實作的部分，會另外增加一個勞退分級表的上傳位置。個人自願提繳率則是在每位同事的人事資料裡設定。
...
### 需要 Linda 確認（薪資簽核流程，回報 `26011df2`）

- **「一個按鍵簽核」是指哪一種？** 我們的理解是「**同一關的多筆一起簽**」——例如按一次「HR 審核」就把當月所有草稿一起送到下一關，而不是每位同事各按一次（7 人 × 4 關 = 28 次點擊）。四道關卡本身保留。
  另一種可能的意思是「**一個按鍵直接從草稿跑到已發薪**」，也就是四關一次完成。這個做法我們**不建議**：四關的意義就是分段覆核，一鍵跑完等於沒有簽核，薪資又是最需要責任歸屬的資料。麻煩您確認是前者，我們就照前者做。
- **第三關要改成「人資長核准」**：目前第三關是「營運長核准」。改成人資長沒有問題，但需要確認兩件事——**誰是人資長**（要指定人選以便設定權限），以及**營運長是否完全退出薪資簽核**。確認後我們調整關卡名稱與權限。

...
### Added（新增）
- **可以撤銷已核准的請假**（回報 `1a3f5591`）：先前只有「待審核」的請假可以取消，已核准的單子員工和管理員都動不了。現在已核准的請假可由**申請人本人**或**人資／管理員**撤銷，撤銷時會自動退還當初扣掉的假別額度、並移除已同步到 Outlook 的行事曆事件；人資代為撤銷時會用 Teams 通知當事人。撤銷會留下操作人、時間與原因的紀錄可供查核。

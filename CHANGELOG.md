...
## v0.4.3 — 每日報告模組上線（2026-06-18）

### 新功能

- **每日報告模組**：業務同仁可在 myOPS 直接填報每日行程、完成回報、KPI 數值，取代原有獨立網站
  - `/每日報告`：每日填報（行程 / 完成回報 / KPI 三個分頁），支援一鍵套用樣板
  - `/我的任務`：查看主管指派的任務與子任務，完成後可標記回報
  - `/團隊總覽`：主管（Viewer）可查看所屬群組所有成員的當日報告
- **群組管理**（管理員）：`/admin/每日報告群組` 可建立群組、指派成員與主管（Viewer），彈性對應各部門結構
- **測試中標示**：各功能頁面標題旁顯示「測試中」提示，提醒目前為測試階段
